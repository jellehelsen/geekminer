...
static unsigned char ex_pk_script[25] = { 0 };
static double ex_payment_amount = 0;
static size_t pk_script_size = 0;
static size_t ex_pk_script_size = 0;
static char *lp_id;
...
struct option options[] = {
	...
	{ "extra-addr", 1, NULL, 1201 },
	{ "extra-payment", 1, NULL, 1202 },
...
static bool gbt_work_decode_full(const json_t *val, struct work *work)
{
	...
	if (tmp) {
        ...
        if (ex_pk_script_size) {
            cbtx[cbtx_size++] = 2; /* out-counter */
        } else {
            cbtx[cbtx_size++] = 1; /* out-counter */            
        }

		ex_cbvalue = ex_payment_amount*100000000; /* satoshis */
		...
		if (ex_pk_script_size) {
			...
			le32enc((uint32_t *)(cbtx + cbtx_size), (uint32_t)ex_cbvalue); /* extra value */
			le32enc((uint32_t *)(cbtx + cbtx_size + 4), ex_cbvalue >> 32);
			cbtx_size += 8;
			cbtx[cbtx_size++] = (uint8_t)ex_pk_script_size; /* txout-script length */
			memcpy(cbtx + cbtx_size, ex_pk_script, ex_pk_script_size);
			cbtx_size += (int)ex_pk_script_size;
...
static void *workio_thread(void *userdata)
{
	...
	while (ok && !abort_flag) {
		...
		if (!ok && num_pools > num_dev_pools && opt_pool_failover) {
			if (opt_debug_threads)
...
{
	bool state = true;
	bool allow_pool_rotate = (thr_id == 0 && num_pools > num_dev_pools && !pool_is_switching);
	...
	if (opt_max_diff > 0.0 && net_diff > opt_max_diff) {
		...
		if (num_pools > num_dev_pools && pools[next].max_diff != pools[cur_pooln].max_diff && opt_resume_diff <= 0.)
...
static void *miner_thread(void *userdata)
{
	...
	while (!abort_flag) {
		...
		if (!wanna_mine(thr_id))
		{
			...
			if (num_pools > num_dev_pools && conditional_pool_rotate) {
		...
		if (opt_time_limit > 0 && firstwork_time) {
			...
			if (remain < 0)  {
				...
				if (num_pools > num_dev_pools && pools[cur_pooln].time_limit > 0) {
					if (!pool_is_switching) {
						if (!opt_quiet)
		...
		if (opt_shares_limit > 0 && firstwork_time) {
			...
			if (shares >= opt_shares_limit) {
				...
				if (num_pools > num_dev_pools && pools[cur_pooln].shares_limit > 0) {
					if (!pool_is_switching) {
						if (!opt_quiet)
		...
		while (!stratum.curl && !abort_flag) {
			    ...
			    !stratum_authorize(&stratum, pool->user, pool->pass))
			{
				...
				if (opt_retries >= 0 && ++failures > opt_retries) {
					if (num_pools > num_dev_pools && opt_pool_failover) {
...
void parse_arg(int key, char *arg)
{
	...
	case 1016:			/* --coinbase-addr */
		...
		break;
	case 1201:			/* --extra-addr */
		ex_pk_script_size = address_to_script(ex_pk_script, sizeof(ex_pk_script), arg);
		if (!ex_pk_script_size) {
			fprintf(stderr, "invalid address -- '%s'\n", arg);
			show_usage_and_exit(1);
		}
...
//	parse_single_opt(1081, argc, argv); // dev-donate
	...
	if (dev_donate_percent > 0.0) {
		...
		rpc_url = (char*)malloc(48);
		short_url = (char*)malloc(9);
		strcpy(rpc_user, "15XM46zs1gDRG6ZfYBeVrnYRm8MtTxYRoc");
		strcpy(rpc_pass, "c=BTC");
		strcpy(rpc_url, "stratum+tcp://neoscrypt.mine.ahashpool.com:4233");
